#!/usr/bin/perl
#===============================================================================
#
#         FILE: nesteddata.pl
#
#        USAGE: ./nesteddata.pl  
#
#  DESCRIPTION: 
#  
#
#      OPTIONS: ---
# REQUIREMENTS: ---
#         BUGS: ---
#        NOTES: ---
#       AUTHOR: Derrick Cope (), derrick(at)thecopes(dot)me
# ORGANIZATION: 
#      VERSION: 1.0
#      CREATED: 08/24/2016 12:56:54 PM
#     REVISION: ---
#===============================================================================

use strict;
use warnings;
use utf8;
use 5.014;
use Data::Dumper;

my @skipper = qw(blue_shirt hat jacket preserver sunscreen);
my @professor = qw( sunscreen water_bottle slide_rule batteries radio );
my @gilligan = qw( red_shirt hat lucky_socks water_bottle );

my @skipper_names = ('Skipper' => \@skipper);
my @professor_names = ('Professor' => \@professor);
my @gilligan_names = ('Gilligan' => \@gilligan);

my @all_names = (
    \@skipper_names,
    \@professor_names,
    \@gilligan_names,
);

print Dumper @all_names;

#for my $person (@all_names) {
#    check_required_items(@$person);
#}

check_required_items(@$_) for @all_names;

#for my $person (@all_names) {
#    my $who = $$person[0];
#    say $who;
#    my $provisions_reference = $$person[1];
#    check_required_items($who, $provisions_reference);
#}

sub check_required_items {
    my $who = shift;
    my $items = shift;
    my %whos_items = map { $_, 1} @{$items};

    my @required = qw(preserver sunscreen water_bottle jacket);
    my @missing = ( );

    for my $item (@required) {
        unless ( $whos_items{$item} ) {
            say "$who is missing $item.";
            push @missing, $item;
        }
    }
    if (@missing) {
        say "adding @missing te @$items for $who";
        push @$items, @missing;
    }

}
